...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, onOpenTerminal, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone, onRunningSessionIdsChange, onSessionsChange, onCloseSidebar }: Props) {
  ...
  // 用 typeof window 守卫以避免 Next.js 服务端预渲染时访问 window 报错。
  const [isRemote, setIsRemote] = useState(() => (
    typeof window !== "undefined" && isRemoteHost(window.location.hostname)
  ));
  useEffect(() => {
    if (typeof window !== "undefined") setIsRemote(isRemoteHost(window.location.hostname));
  }, []);
  ...
  // Git 提交区块在 isGitRepository 异步就绪后才渲染，轮询等待其出现再滚动。
  const scrollToGitChanges = () => {
    const target = () => document.getElementById("pi-git-changes");
...
const handleOpenDirectoryInFileManager = useCallback(() => {
    const dir = selectedCwd ?? selectedCwdProp;
    if (!dir) return;
    void fetch(`/api/files/${encodeFilePathForApi(dir)}?type=open-directory`, { method: "POST" })
      .catch(() => {});
...
{onOpenTerminal && (
            ...
            <ToolbarIconButton
              onClick={() => {
                ...
                const expanding = changesCollapsed;
                setChangesCollapsed(!expanding);
                if (expanding) scrollToGitChanges();
              }}
              title={t("sidebar.gitPanel")}
